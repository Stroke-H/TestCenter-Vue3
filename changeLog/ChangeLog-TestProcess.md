...
### Added
- **Project Tree Dual-View Configuration Overhaul**: 项目树配置追踪重构为结构化双重视角工作台，彻底解决杂乱便签难以梳理、版本与配置脱节的痛点。
  - **当前生效配置全景视图 (`ProjectConfigActiveView`)**：提供当前版本生效配置的清晰总览，支持按分类与群体即时过滤；高亮当前参数值与历史变更 Diff（旧值 ➔ 当前值），展示适用端与来源版本胶囊，并提供历史废除配置折叠专区。
  - **版本引入与演进时间轴 (`ProjectConfigTimelineView`)**：左侧展示各版本变动指标（`+新增`、`~变更`、`-废除`），右侧呈现该版本配置变更 Changelog 卡片，清晰列出新引入项、参数调整、废弃项以及验收报告依据。
  - **版本树双向联动打通 (`ProjectTreeBoard`)**：下方版本卡片头部自动标识该版本对应的配置变动胶囊（例如 `🎯 2 项配置变动`），点击直达上方时间轴详情。
  - **经典便签无损兼容**：完整保留原有彩色便签墙、拖拽排序、即时编辑与一键跨项目粘贴功能，提供一键切换开关。

## 2026-08-18

### Added
- **Project Config Topic Index**: “全部项目配置”新增便签主题标签聚合；选择标签后会列出相关项目、保留项目的完整配置记录并高亮匹配项，点击项目可平滑定位到对应配置区块。
...
### Added
- **TTmins Memo Sync**: 项目树会参考 `A1160` 项目的前四条项目配置便签，自动为所有项目名或项目代码包含 `TTmins` 的项目补齐同内容便签，包括“前两集免费”配置；已有同内容便签不会重复添加。
- **Project Memo History And Sorting**: 项目配置便签新增修改历史记录，悬浮便签可查看修改前内容与修改时间；便签支持按文案长度自适应宽度、自动换行排布，并可通过鼠标左键拖拽调整顺序。
