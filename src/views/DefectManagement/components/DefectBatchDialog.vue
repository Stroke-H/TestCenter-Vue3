...
const emit = defineEmits<{
  ...
  execute: [payload: Omit<DefectBatchPayload, "ids">]
}>()

const form = reactive<Omit<DefectBatchPayload, "ids">>({
  action: "assign",
  assignee_id: "",
  severity: 3,
  priority: 3,
...
watch(() => props.modelValue, (visible) => {
  if (visible) {
    Object.assign(form, {
      action: canProcess.value ? "assign" : canVerify.value ? "close" : canReopen.value ? "reopen" : "archive",
      assignee_id: "",
      severity: 3,
      priority: 3,
      resolution: "fixed",
...
<template>
  <el-dialog
    ...
    width="540px"
    destroy-on-close
    class="defect-batch-dialog"
    @close="emit('update:modelValue', false)"
  >
    <div class="batch-hint">
      <span class="batch-hint__icon"><el-icon><Operation /></el-icon></span>
      <div>
        <strong>已选择 {{ count }} 条缺陷</strong>
        <p>系统会逐条检查生命周期状态与项目权限，仅符合条件的记录会被更新。</p>
      </div>
    </div>
    ...
    <el-form v-else label-position="top" class="batch-form">
      <el-form-item label="批量操作类型">
        <el-select v-model="form.action" class="full-width">
          <el-option-group v-if="canProcess" label="属性调整">
            <el-option label="指派处理人" value="assign" />
            <el-option label="修改严重程度" value="severity" />
            <el-option label="修改优先级" value="priority" />
          </el-option-group>
          <el-option-group label="生命周期">
            <el-option v-if="canProcess" label="确认缺陷" value="confirm" />
            <el-option v-if="canProcess" label="标记解决" value="resolve" />
            <el-option v-if="canVerify" label="验证关闭" value="close" />
            <el-option v-if="canReopen" label="重新激活" value="reopen" />
          </el-option-group>
          <el-option-group v-if="canArchive" label="管理操作">
            <el-option label="归档缺陷" value="archive" />
          </el-option-group>
        </el-select>
      </el-form-item>

      <el-form-item v-if="form.action === 'assign' || form.action === 'confirm'" label="处理人" :required="form.action === 'assign'">
        <el-select v-model="form.assignee_id" clearable filterable class="full-width" placeholder="选择处理人">
          <el-option
      ...
      <el-form-item v-if="form.action === 'severity'" label="严重程度">
      ...
      <el-form-item v-if="form.action === 'priority'" label="优先级">
      ...
      <template v-if="form.action === 'resolve'">
        ...
        <el-form-item label="解决版本">
          <DefectVersionSelect v-model="form.resolved_version" :meta="meta" :projects="[...new Set(defects.map(item => item.project_code))]" />
        </el-form-item>
      </template>

      <el-form-item label="操作备注" :required="needsComment">
        <el-input
          ...
          maxlength="1000"
          show-word-limit
          :placeholder="needsComment ? '重新激活必须填写具体原因' : '可填写本次批量操作的补充说明'"
        />
      </el-form-item>

      <div v-if="form.action === 'archive'" class="archive-warning">
        归档后缺陷将不再出现在普通列表中，但操作历史和缺陷数据仍会完好保留。
      </div>
    </el-form>
    ...
    <template #footer>
      ...
      <el-button
        :type="form.action === 'archive' ? 'danger' : 'primary'"
        :disabled="!canSubmit"
        :loading="loading"
        @click="emit('execute', { ...form })"
      >
        执行批量操作
      </el-button>
    </template>
