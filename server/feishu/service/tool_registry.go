	...
	RegisterTool(ToolDef{
		...
		Execute: func(ctx context.Context, chatID string, senderID string, argsJSON string) (string, error) {
			...
			return fmt.Sprintf("✅ 找到项目信息：\n"+
				...
				"平台：%s\n\n"+
				"【核心查询指令 ★★★ 必须严格执行 ★★★】：\n"+
				"1. 获取需求：请调用 search_by_mql，参数 mql 为：%s 并且请你自行在后面加上版本过滤条件（例如：AND `规划版本` = 'v%s'，如果用户已输入 v 前缀则不要重复添加）。\n"+
				"2. 获取缺陷：请调用 search_by_mql，参数 mql 为：%s 并且请你自行在后面加上版本过滤条件（例如：AND `解决版本` = 'v%s'，如果用户已输入 v 前缀则不要重复添加）。\n"+
